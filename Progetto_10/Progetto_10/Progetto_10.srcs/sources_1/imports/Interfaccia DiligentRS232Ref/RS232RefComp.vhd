...
--  ----------------------------------------------------------------------------------------
	...
	process (tClk, RST) -- qui si aggiorna lo stato e basta
		begin
			if (tClk = '1' and tClk'Event) then --ricorda che tclock ? il bit pi? significativo di quel registro contatore
				if RST = '1' then
					sttCur <= sttIdle;
				else
					sttCur <= sttNext;
	...
	process (stbeCur, CLK, WR, DBIN, load)
		begin
			...
			case stbeCur is
				...
				when stbeIdle => -- ma questo ? NUTS
				--cio?, aspetta che WR=0 perch? ho scritto, poi
				-- metti tbe=1 per generare l'interrupt
				-- poi aspetta WR per tornare a 0;
				-- se non torna a 0 l'altro process non pu? partire 
					TBE <= '1';
					if WR = '1' then
						stbeNext <= stbeSetTBE;
					else
